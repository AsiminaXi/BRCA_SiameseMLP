...
3. **Stage 3 — Feature Extraction (Frozen ESM-MSA-1b):** Pads or trims homologs to query length (max 512 seqs, 1022 cols). Extracts the mean per-residue embedding from layer 12 `(768,)`. It also computes the **Site-Specific $\Delta$LLR** using only the WT context: `log P(WT) - log P(MUT)`.
